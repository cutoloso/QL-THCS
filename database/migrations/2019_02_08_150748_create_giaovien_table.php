...
class CreateGiaovienTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('GiaoVien', function (Blueprint $table) {
            ...
            $table->string('gv_email')->comment('Email giáo viên');
            $table->string('gv_dienThoai',10)->comment('Số điện thoại liên lạc');
            $table->char('cm_ma',8)->comment('Mã chuyên môn');
            $table->char('q_ma',8)->comment('Mã quyền');
            // $table->timestamp('gv_taoMoi')->default(DB::raw('CURRENT_TIMESTAMP'))->comment('Thời điểm tạo mới');

            // $table->timestamp('gv_capnhat')->default(DB::raw('CURRENT_TIMESTAMP'))->comment('Thời điểm cập nhật gần nhất');
        // khóa            
            $table->primary(['gv_ma']);
