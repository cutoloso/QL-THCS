app.controller('HocSinhDiemController',function($scope,$http,URL_Main){
	...
	function fillData(){
		$http.get('http://localhost/QL-THCS/public/ket-qua/'+ $scope.hs_ma +'/hs-ma/'+ $scope.hk_hocKy+'/hoc-ky/'+$scope.mh_ma+'/ma-mh').then(function(response){
			$scope.ds_kq = response.data.message.ds_KetQua;
		}).then($http.get('http://localhost/QL-THCS/public/ket-qua/diemTBCN/'+ $scope.hs_ma + '/hs-ma/'+ $scope.hk_hocKy+'/hoc-ky/'+$scope.mh_ma+'/ma-mh')
