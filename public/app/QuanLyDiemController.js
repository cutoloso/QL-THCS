app.controller('QuanLyDiemController',function($scope,$http,URL_Main){
	...
	$scope.modal = function(state, hs_ma, mh_ma, kq_diem, kq_heSo, kq_lan){
		...
		switch(state){
			case "add":
			$scope.hs_ma = hs_ma;
			$http.get('http://localhost/QL-THCS/public/ket-qua/'+ hs_ma +'/hs-ma/'+ $scope.hk_hocKy+'/hoc-ky/'+$scope.gv.cm_ma+$scope.khoi+'/ma-mh').then(function(response){
				$scope.ds_kq = response.data.message.ds_KetQua;
			}).then($http.get('http://localhost/QL-THCS/public/ket-qua/diemTBCN/'+ hs_ma + '/hs-ma/'+ $scope.hk_hocKy+'/hoc-ky/'+$scope.gv.cm_ma+$scope.khoi+'/ma-mh')
